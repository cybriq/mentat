...
// GetStake returns the divider that makes the Hash of the Header equal to or
// below the value generated by the Difficulty, which is locked up for the cool
// down period (SaphoConsensus.StakeCoolDown).
//
...
func (h *Header) GetStake() (stake uint64) {
	...
	if target.Cmp(proof) < 0 {
		divider, mod := new(big.Int).DivMod(target, proof, new(big.Int))
		stake = divider.Uint64()
